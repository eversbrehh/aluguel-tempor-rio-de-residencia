...
export class OutboxRepository {
  ...
  async markPublished(eventId: string): Promise<void> {
    const { error } = await supabaseAdmin.rpc('mark_outbox_published', {
      p_event_id: eventId,
    });
    if (error) throw new Error(`mark_outbox_published failed: ${error.message}`);
  }

  async markFailed(eventId: string, errorMessage: string, maxAttempts: number): Promise<void> {
    const { error } = await supabaseAdmin.rpc('mark_outbox_failed', {
      p_event_id: eventId,
      p_error: errorMessage,
      p_max_attempts: maxAttempts,
    });
    if (error) throw new Error(`mark_outbox_failed failed: ${error.message}`);
  }
}
