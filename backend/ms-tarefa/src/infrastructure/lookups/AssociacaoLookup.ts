...
/**
 * Acesso somente-leitura às tabelas do monolito (associacoes + imoveis).
 * Necessário para validar autorização e descobrir as partes envolvidas.
 *
 * Estratégia justificável: ambos os componentes compartilham o mesmo
 * cluster Supabase (monolito modular + microsserviços), conforme
 * diagrama da Documentação LAMD.
 */
export class AssociacaoLookup {
  async byId(associacaoId: string): Promise<AssociacaoLookupResult> {
    ...
    // Supabase tipifica join FK como array. Pegamos o primeiro (1:1 lógico).
